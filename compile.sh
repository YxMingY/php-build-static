...
EXT_SCERIO_CHUNKUTILS_VERSION="master"
...
while getopts "::t:oj:srcdlxzff:ugn" OPTION; do
	...
	case $OPTION in
		...
		f)
			...
			;;
		u)
			echo "[opt] Will compile with SCERIO-ChunkUtils C extension for Anvil"
			COMPILE_SCERIO_CHUNKUTILS="yes"
			;;
		g)
...
if [ "$IS_CROSSCOMPILE" == "yes" ]; then
	...
	if [[ "$COMPILE_TARGET" == "win" ]] || [[ "$COMPILE_TARGET" == "win64" ]]; then
		...
		echo "Please supply a proper platform [mac win win64 android-aarch64 android-armv7] to cross-compile"
		exit 1
	fi
...
#TODO: add aarch64 platforms (ios, android, rpi)
	...
	if [ `getconf LONG_BIT` == "64" ]; then
		...
		echo "[ERROR] SCERIO-MP is no longer supported on 32-bit systems"
		exit 1
	fi
fi
...
if [ "$COMPILE_SCERIO_CHUNKUTILS" == "yes" ]; then
	get_github_extension "SCERIO-chunkutils" "$EXT_SCERIO_CHUNKUTILS_VERSION" "dktapps" "SCERIO-C-ChunkUtils"
	HAS_SCERIO_CHUNKUTILS=--enable-SCERIO-chunkutils
else
	HAS_SCERIO_CHUNKUTILS=""
fi
...
if [ "$COMPILE_FOR_ANDROID" == "yes" ]; then
	sed -i=".ba>ckup" 's/-export-dynamic/-all-static/g' Makefile
fi
